...
class PreCoBlock(nn.Module):
    ...
    def forward(self, x, residual=None, inference_params=None, ttt_lr_mult=1.0):
        ...
        position_score = A.mean(dim=1)  # [B, L]
        
        # 2. 記憶偏移檢測
        # 計算當前位置與前面位置的語意一致性
        if h_hidden_reduced.size(1) > 1:
            # 計算與前一個位置的語意距離
            current_features = h_hidden_reduced[:, 1:, :]  # [B, L-1, d]
            prev_features = h_hidden_reduced[:, :-1, :]    # [B, L-1, d]
            semantic_distance = torch.norm(current_features - prev_features, dim=-1)  # [B, L-1]
            
            # 檢測語意轉折點（距離突然增大）
            semantic_shift = torch.sigmoid(semantic_distance - semantic_distance.mean(dim=-1, keepdim=True))
            semantic_shift = F.pad(semantic_shift, (1, 0), value=0.0)  # [B, L]
        else:
            semantic_shift = torch.zeros_like(position_score)
        
        # 3. TTT 校正效果評估
        correction_signal = z_ttt - z_longhorn
        correction_magnitude = torch.norm(correction_signal, dim=-1)  # [B, L]
        
        # 4. 綜合 gain 計算
        # 結合位置重要性、語意轉折、校正效果
        raw_gain = torch.sigmoid(
            self.alpha * position_score +           # TokenButler 位置感知
            self.beta * semantic_shift +            # 語意轉折檢測
            (1.0 - self.alpha - self.beta) * correction_magnitude  # 校正效果
        )
        ...
        # 5. 自適應校正應用
        # 在語意轉折點增強 TTT 校正
        adaptive_gain = gain * (1.0 + semantic_shift.unsqueeze(-1))
        out = z_longhorn + adaptive_gain * correction_signal
    ...
    def _ttt_training_forward(self, hidden_states, ttt_lr_mult=1.0):
        ...
        # 改進的自監督目標：基於校正效果
        # 目標：讓 TTT 學習如何校正 Longhorn 的輸出
        # 使用 XK 作為自監督目標（保持維度一致）
        ssl_target = XK  # 讓 TTT 學習與輸入 K 一致的表示
        grad_l = Z1_normed - ssl_target  # 梯度：TTT 輸出與輸入 K 的差異
