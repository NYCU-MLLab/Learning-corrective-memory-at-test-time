...
class LonghornConfig:
    # 🔧 修正：使用 run.sh 中 longhorn 的參數設置
    d_model: int = 768  # 對應 --n_embd=512
    d_intermediate: int = 0
    n_layer: int = 12  # 對應 --n_layer=12
    vocab_size: int = 50277  # 保持原有設置
...
class LonghornLM(nn.Module):
    def __init__(self, config: LonghornConfig):
        super().__init__()
        # 🔧 修正：如果 ssm_cfg 已經設定，直接使用；否則使用預設值
        if not config.ssm_cfg:
            config.ssm_cfg = {
                "d_state": 16,
                "d_conv": 4,
                "expand": 2
    ...
    def forward(self, idx, targets=None, return_hidden=False):
        hidden_states = self.transformer.backbone(idx)
