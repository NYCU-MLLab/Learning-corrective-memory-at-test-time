        ...
        if causal_conv1d_fn is None:
            if cache_params is not None:
                if cache_params.seqlen_offset > 0:
                    ...
                    cache_params.conv_states_dic["pre_conv"][self.layer_idx].copy_(conv_state.contiguous())
...
class TTTBase(nn.Module):
    def __init__(self, config: TTTConfig, layer_idx: Optional[int] = None):
        ...
        self.n_mini_batch = self.max_sequence_length // self.mini_batch_size
    ...
    def _split_mini_batches(self, hidden_states):
        """將 [B, H, L, D] 固定切分成 [B, H, n_mini_batch, mini_batch_size, D]"""
        B, H, L, D = hidden_states.shape
        assert L == self.max_sequence_length, (
            "sequence length must equal max_position_embeddings for fixed split. "
            "請使用固定長度的 dataloader（等於 max_position_embeddings）或在層內先 pad/截斷後再切分。"
        )
        n_mb = self.n_mini_batch
        mb = self.mini_batch_size
        return hidden_states.reshape(B, H, n_mb, mb, D)
    ...
    def get_eta(self, X, mini_batch_step_offset, mini_batch_size):
        ...
        # [B, num_heads, num_mini_batch, mini_batch_size, 1] - 保持原形狀
        ttt_lr_eta = self.config.ttt_base_lr * ttt_lr / self.head_dim

        # [B, L] 以循環方式擷取 mini-batch 內的位置索引，確保長度恆為 mini_batch_size
        token_idx_full = self.token_idx + self.learnable_token_idx  # [mini_batch_size]
        offset = int(mini_batch_step_offset % self.mini_batch_size)
        if mini_batch_size == self.mini_batch_size and offset == 0:
            # 完整一個 mini-batch 且從頭開始，直接用整段
            token_idx = token_idx_full
        else:
            # 取長度為 mini_batch_size 的片段（支援 wrap-around）
            end = offset + mini_batch_size
            if end <= self.mini_batch_size:
                token_idx = token_idx_full[offset:end]
            else:
                first = token_idx_full[offset:]
                second = token_idx_full[: end - self.mini_batch_size]
                token_idx = torch.cat([first, second], dim=0)
        ...
        token_eta = torch.broadcast_to(
            ...
            (X.shape[0], self.num_heads, self.n_mini_batch, mini_batch_size, 1),
        )
    ...
    def get_ttt_inputs(self, inputs, mini_batch_size, cache_params, ttt_lr_mult: Optional[float] = 1.0):
        XQ = inputs["XQ"]
        XK = inputs["XK"]
        XV = inputs["XV"]
        X = inputs["X"]
        ...
        # apply external multiplier (default 1.0) for parity with JAX implementation
        if ttt_lr_mult is not None:
            if not torch.is_tensor(ttt_lr_mult):
                ttt_lr_mult = torch.tensor(ttt_lr_mult, dtype=eta.dtype, device=eta.device)
            eta = eta * ttt_lr_mult
    ...
    def get_ttt_inputs_fixed(self, inputs, cache_params, ttt_lr_mult: Optional[float] = 1.0):
        """🔧 新增：固定切分的TTT輸入準備 (匹配JAX版本)"""
        XQ = inputs["XQ"]  # [B, num_heads, n_mini_batch, mini_batch_size, head_dim]
        XK = inputs["XK"]
        XV = inputs["XV"]
        X = inputs["X"]  # [B, max_sequence_length, hidden_size]
        
        B = X.shape[0]
        # 重塑X以匹配固定切分
        X = X.reshape(B, *self.seq_shape, self.width)
        ...
        token_eta, ttt_lr_eta = self.get_eta(X, mini_batch_step_offset, self.mini_batch_size)
        eta = token_eta * ttt_lr_eta
        # apply external multiplier (default 1.0) for parity with JAX implementation
        if ttt_lr_mult is not None:
            if not torch.is_tensor(ttt_lr_mult):
    ...
    def forward(
        ...
        # === 判斷是否在增量解碼/生成 ===
        # 僅當 L==1 且有 cache 時才視為解碼；L>1 (prefill) 仍走固定切分路徑
        is_decoding = (cache_params is not None) and (L == 1)

        if is_decoding:
            # -------- 串流/解碼路徑：完全不補、不固定切分（L==1），單步前向 --------
            assert position_ids is not None, "position_ids should be provided by TTTModel in decoding"
            # 保存原始 offset，稍後還原，避免干擾 TTTModel 的全域 offset 累加
            orig_offset = cache_params.seqlen_offset if cache_params is not None else 0
            # 讓底層 conv/update 分支知道「已經有歷史」，從第二個 token 開始用 update path
            if cache_params is not None:
                cache_params.seqlen_offset = orig_offset

            hs_t = hidden_states  # [B, 1, C]
            pos_t = position_ids  # [B, 1]

            XQ, XK, XV = self.get_qkv_projections(hs_t, cache_params=cache_params)
            # [B, 1, C] -> [B, 1, num_heads, head_dim] -> [B, num_heads, 1, head_dim]
            XQ = XQ.reshape(B, 1, self.num_heads, self.head_dim).transpose(1, 2)
            XK = XK.reshape(B, 1, self.num_heads, self.head_dim).transpose(1, 2)
            XV = XV.reshape(B, 1, self.num_heads, self.head_dim).transpose(1, 2)

            cos, sin = self.rotary_emb(XV, (pos_t % self.mini_batch_size))

            XQ, XK = permute_qk(XQ, XK)
            XQ, XK = apply_rotary_pos_emb(XQ, XK, cos, sin)
            XQ, XK = undo_permute_qk(XQ, XK)

            # 組裝成單步 inputs，並用 mini_batch_size=1 走 dual-form 更新
            inputs = {
                "XQ": XQ,  # [B, nh, 1, 1, d]
                "XK": XK,
                "XV": XV,
            ...
            ttt_inputs = self.get_ttt_inputs(
                inputs, mini_batch_size=1, cache_params=cache_params, ttt_lr_mult=ttt_lr_mult
            )

            out_t, _ = self.ttt(
                ttt_inputs,
                mini_batch_size=1,
                last_mini_batch_params_dict=None,     # 由 ttt() 內自行從 cache 取
                cache_params=cache_params,
            )  # [B, 1, C]

            # 極低頻檢查（僅第一次步驟時觸發）：offset 與 token_idx 是否依序循環
            if orig_offset == 0:
                # 檢查 seqlen_offset 單步增長（這裡應該尚未累加，結束後由 TTTModel 統一加 1）
                assert cache_params is not None and cache_params.seqlen_offset == 0, \
                    "解碼首步 offset 應為 0"
                # 檢查 token_idx 從 1, 1/2, ... 開始（允許有 learnable 偏移）
                with torch.no_grad():
                    token_idx_probe = (self.token_idx + self.learnable_token_idx).clamp_min(0.0)
                    assert token_idx_probe[0] >= token_idx_probe[1], \
                        "token_idx 應沿序列遞減（例如 1, 1/2, ...）"

            # 還原 offset，讓 TTTModel 在整個 forward 結束後統一加上 L
            if cache_params is not None:
                cache_params.seqlen_offset = orig_offset
            output_hidden_states = out_t  # [B, 1, C]
        ...
        # -------- 非解碼（訓練/整段評估）路徑：固定切分 + 補到 max_sequence_length --------
        # 確保序列長度是固定長度：不足補 0，超過截尾
        if L != self.max_sequence_length:
            if L > self.max_sequence_length:
                hidden_states = hidden_states[:, -self.max_sequence_length:]
                L = self.max_sequence_length
            else:
                pad_len = self.max_sequence_length - L
                hidden_states = torch.cat(
                    [
                        hidden_states,
                        torch.zeros(
                            B, pad_len, hidden_states.shape[-1],
        ...
        # position_ids 對齊
        if position_ids is None or position_ids.shape[-1] != L:
        ...
        inputs = {"XQ": XQ, "XK": XK, "XV": XV, "X": hidden_states}

        output_hidden_states, _ = self.ttt(
            self.get_ttt_inputs_fixed(inputs, cache_params, ttt_lr_mult=ttt_lr_mult),
        ...
        # 裁回原始長度
        if output_hidden_states.shape[1] != original_L:
            output_hidden_states = output_hidden_states[:, :original_L, :]
        ...
        return output_hidden_states
        # 🔧 修改：使用固定切分 (匹配JAX版本)
        # 確保序列長度是mini_batch_size的倍數
        # if L != self.max_sequence_length:
            # 如果序列長度不匹配，進行padding或截斷
            # if L > self.max_sequence_length:
                # 保留尾端，與滑動視窗/生成時行為一致
                # hidden_states = hidden_states[:, -self.max_sequence_length:]
                # L = self.max_sequence_length
            # else:
                ...
                # hidden_states = torch.cat([
                    # hidden_states,
                    # torch.zeros(B, pad_len, hidden_states.shape[-1], 
                            #   device=hidden_states.device, dtype=hidden_states.dtype)
                # ], dim=1)
                # L = self.max_sequence_length

        # 確保 position_ids 與當前序列長度 L 一致，避免 RoPE 長度不匹配
        # if position_ids is None or position_ids.shape[-1] != L:
            # position_ids = torch.arange(0, L, dtype=torch.long, device=hidden_states.device).unsqueeze(0)
# 
        # XQ, XK, XV = self.get_qkv_projections(hidden_states, cache_params=cache_params)
...
# 
        ...
        # 使用固定切分進行TTT計算
        # output_hidden_states, _ = self.ttt(
            # self.get_ttt_inputs_fixed(inputs, cache_params),
            # mini_batch_size=self.mini_batch_size,
            # last_mini_batch_params_dict=None,
            # cache_params=cache_params,
        # )

        # 將輸出裁回原始長度，避免與殘差維度不一致
        # if output_hidden_states.shape[1] != original_L:
            # output_hidden_states = output_hidden_states[:, :original_L, :]
...
class TTTLinear(TTTBase):
    ...
    def ttt(
        ...
        # 🔧 修改：支援固定切分 (匹配JAX版本)
        # [B, num_heads, n_mini_batch, mini_batch_size, head_dim]
        B = inputs["XV"].shape[0]
        num_mini_batch = inputs["XV"].shape[2]  # 固定為 n_mini_batch
        L = inputs["XV"].shape[2] * inputs["XV"].shape[3]  # n_mini_batch * mini_batch_size
        device = inputs["XV"].device
        dtype = inputs["XV"].dtype

        # 🔧 由 config 控制是否使用對偶式（預設 True，便於 ablation）
        use_dual_form = bool(self.config.use_dual_form)
        ...
        def compute_mini_batch(params_dict, inputs):
            ...
            ln_weight = self.ttt_norm_weight.unsqueeze(1)  # [num_heads, 1, head_dim]
            ln_bias = self.ttt_norm_bias.unsqueeze(1)      # [num_heads, 1, head_dim]
            # [B,nh,K,f]
            ...
            if use_dual_form:
                ...
                # 前綴和（沿 K 維）加權累積梯度：等價於 tril(ones) @ (eta * grad)
                # [B,nh,K,f]
                b1_bar = b1_init - torch.cumsum(eta_mini_batch * grad_l_wrt_Z1, dim=-2)
        ...
        # 🔧 修改：固定切分的輸出處理 (匹配JAX版本)
        # [B,num_heads, n_mini_batch, mini_batch_size, f] -> [n_mini_batch, B, num_heads, mini_batch_size, f]
        ...
        # XQW_batch: [n_mini_batch, B, num_heads, mini_batch_size, head_dim]
        ...
        # [n_mini_batch, B, num_heads, mini_batch_size, head_dim] -> [B, n_mini_batch, mini_batch_size, num_heads, head_dim]
...
class TTTMLP(TTTBase):
    ...
    def ttt(
        ...
        def compute_mini_batch(params_dict, inputs):
            ...
            reconstruction_target = XV_mini_batch - X1
            ...
            if use_dual_form:
                ...
                # 前綴和（沿 K 維）加權累積梯度：等價於 tril(ones) @ (eta * grad)
                # [B,nh,K,f]
                b2_bar = b2_init - torch.cumsum(eta_mini_batch * grad_l_wrt_Z2, dim=-2)
...
class TTTModel(TTTPreTrainedModel):
    ...
    def forward(
        ...
        # if (input_ids is None) ^ (inputs_embeds is not None):
            ...
            # )
        # 兩者同時為 None 或同時非 None 才拋錯
        if (input_ids is None) == (inputs_embeds is None):
           raise ValueError(
               "You must specify exactly one of input_ids or inputs_embeds (not both, not neither)."
           )
        ...
        if attention_mask is None:
            if input_ids is not None:
               ...
               attention_mask = torch.ones((B, L), device=inputs_embeds.device, dtype=torch.long)
    ...
    def forward(
        ...
        *,
        ...
        if labels is not None:
            ...
            # Enable model parallelism and ensure correct dtype
